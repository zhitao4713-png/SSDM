"""
===============================================================================
SSDM Latent Diffusion Pipeline Integration
===============================================================================
This module extends the standard Stable Diffusion Pipeline to seamlessly integrate
Stationary Wavelet Transform (SWT) based watermarking operations directly within
the latent space.

It handles both the forward generation (injection) and DDIM inversion (extraction)
processes efficiently without decoding to the pixel domain during iterations.
"""
...
class SSDMLatentPipeline(StableDiffusionPipeline):
    def __init__(self,
                 vae,
                 text_encoder,
                 tokenizer,
        ...
        super(SSDMLatentPipeline, self).__init__(vae,
                                                 text_encoder,
                                                 tokenizer,
                                                 unet,
                                                 scheduler,
    ...
    def decode_latents_to_tensor(self, latents):
        """Decodes latent representations back into pixel-space image tensors."""
    ...
    def convert_tensor_to_numpy(self, tensor):
        """Converts PyTorch image tensors to NumPy arrays for PIL compatibility."""
        return tensor.detach().cpu().permute(0, 2, 3, 1).float().numpy()

    # Accept keywords args in torch.checkpoint
    def ssdm_unet_forward(self,
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            # 8. Post-processing
            image = self.decode_latents_to_tensor(latents)
            has_nsfw_concept = None
        else:
            # 8. Post-processing
            image = self.decode_latents(latents)
...
class SSDMDetectPipeline(SSDMLatentPipeline):
    def __init__(self,
                 vae,
                 text_encoder,
                 tokenizer,
        ...
        super(SSDMDetectPipeline, self).__init__(vae,
                                                 text_encoder,
                                                 tokenizer,
                                                 unet,
                                                 scheduler,
                                                 ...
                                                 feature_extractor,
                                                 requires_safety_checker)
        self.ssdm_ddim_generation = partial(
            self.ssdm_ddim_inversion, reverse_process=True)
    ...
    @torch.inference_mode()
    def get_image_latents(self, image: torch.Tensor,
                          sample=True, rng_generator=None):
        image = 2.0 * image - 1.0
    ...
    def ssdm_ddim_inversion(
        ...
        **kwargs,
        ...
        # It's more optimized to move all timesteps to correct device
        # beforehand
        ...
        for i, t in enumerate(self.progress_bar(
