...
SSDM Utilities
==============
Helper functions for the Stationary Wavelet Diffusion Watermarking pipeline.
"""
...
def show_latent_and_final_img(latent: torch.Tensor, img: torch.Tensor, pipe):
    with torch.no_grad():
        ...
        pil_img = pipe.numpy_to_pil(pipe.convert_tensor_to_numpy(img))[0]
...
def compute_ssim(a, b):
    return compute_ssim(a, b, data_range=1.).item()
...
def watermark_prob(img, dect_pipe, wm_pipe, text_embeddings, spectral_ring=True, device=torch.device("cuda"), **kwargs):
    ...
    ssdm_reversed_latents = dect_pipe.ssdm_ddim_generation(
        latents=img_latents,
        text_embeddings=text_embeddings,
        guidance_scale=1.0,
        num_inference_steps=50,
    )
    # Pass kwargs (e.g., external_jnd_mask) to the evaluation function
    det_prob = wm_pipe.one_minus_p_value(ssdm_reversed_latents, **kwargs) if not spectral_ring else wm_pipe.spectral_ring_p_value(ssdm_reversed_latents)
    return det_prob
